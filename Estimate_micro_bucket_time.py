...
def  group_degrees_buckets(degrees_group, num_split,step, parameters):
	...
	lctime = (time022-time021)
	time_list=[ptime, ftime, lctime,lbtime] # [preparing time, forward time, loss backward time]
...
def run_split_degree_bucket(degree_split, num_split, step, parameters):
	...
	loss_sum = []
	time_list= []
	ptime,ftime,lctime,lbtime,mtime,pltime = 0,0,0,0,0,0
	buckets_input_num = 0
	...
	if num_split == 1:
		...
		time_list=[ptime, ftime, lctime, lbtime] # [preparing time, forward time, loss backward time]
		print('preparing time: ', ptime)
		print('model forward time : ', ftime)
		print('loss calculation time: ', lctime)
	...
	for step in range(num_split):
		...
		lctime = lctime + (time022-time021)
		lbtime = lbtime + (time03-time003)

		pltime = pltime + (time003-time022) + (time021-time02)
		...
		for epoch in range(args.num_epochs):
			...
			for step_out, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				if epoch >= args.log_indent:
					...
					pure_t =0
					if time_sp_list:
						pure_t += time_sp_list[1]+time_sp_list[2]+time_sp_list[3]
					if time_gp_list:
						pure_t += time_gp_list[1]+time_gp_list[2]+time_gp_list[3]
